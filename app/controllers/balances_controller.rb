class BalancesController < ApplicationController
  ...
  private
    ...
    def set_balance
      @balance = Balance.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def balance_params
      params.require(:balance).permit(:name, :description, :creation_date, :creator_id, participant_ids: []).tap do |balance_params|
        balance_params[:participant_ids].reject!(&:blank?) if balance_params[:participant_ids]
      end
    end
end
